...
st.set_page_config(page_title="Dashboard de Custos - FKM Gritsch", layout="wide", page_icon="🚚")
...
def exibir_dashboard_executivo(df_filtrado, titulo_principal):
    ...
    with col1:
        ...
        st.plotly_chart(fig_top_veiculos, use_container_width=True)
...
if not df.empty:
    ...
    if selected == "Dashboard Executivo":
        ...
        if df_filtrado.empty:
            ...
            st.dataframe(df_detalhado[ordem_colunas_detalhado], use_container_width=True, hide_index=True,
                        column_config={
                            "Custo Total": st.column_config.NumberColumn(format="R$ %.2f"),
        ...
        if df_filtrado.empty:
            ...
            st.dataframe(df_veiculos[ordem_colunas], use_container_width=True, hide_index=True,
                        column_config={
                            "Custo Total": st.column_config.NumberColumn(format="R$ %.2f"),
        ...
        if df_filtrado.empty:
            ...
            if 'TP.Comb' in df_filtrado.columns:
                ...
                st.plotly_chart(fig_combustivel_tipo, use_container_width=True)
            ...
            st.dataframe(df_combustivel_veiculos[ordem_colunas_comb], use_container_width=True, hide_index=True,
                        column_config={
